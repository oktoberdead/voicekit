...
public sealed record ComparisonAudio(float[] Dry, float[] A, float[] B, double GainDry, double GainA, double GainB)
{
...
public static class CalibrationRenderer
{
        ...
        CalibrationSection excerpt, bool matchLevels, CancellationToken ct = default)
    {
        ...
        int end = Math.Min(raw.Length, checked(excerpt.EndSeconds * CalibrationScript.SampleRate));
        ...
        if (carrier is null && (NeedsCarrier(a) || NeedsCarrier(b)))
            throw new InvalidOperationException("В пресете включён файловый вокодер. Нужна его загруженная несущая, либо выбери нейтральный A и «В B оставить только питч».");
...
public sealed class ComparisonPlayback
{
    ...
    public int ReadStereo(Span<float> buffer)
    {
        ...
        {
            for (int v = 0; v < 3; v++) _weights[v] += ((v == variant ? 1 : 0) - _weights[v]) * .004f;
            _volume += (volume - _volume) * .002f;
            int cursor = position + i;
            float sample = _audio.Dry[cursor] * _weights[0] + _audio.A[cursor] * _weights[1] + _audio.B[cursor] * _weights[2];
